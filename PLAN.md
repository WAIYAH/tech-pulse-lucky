...
## 29. Outstanding Manual Steps — UPDATE: done

**All migrations are now applied to a real, live Supabase project** (project "TechPulseInsider", ref `erlqztdknpcyytwzwjwu`, eu-west-2). What actually happened, for the record:

1. The `.env` this repo had been running against (`bsluaknjhnelmzmkncpq`) was **not the user's own project** — a real, reachable Supabase instance, but with zero tables from any of the three migration sets (not just phase 9 — phase 7 and phase 8 had never been applied anywhere either). Confirmed by querying the live REST API directly rather than assuming.
2. A new project was created under the user's own Supabase org, and all four migrations (`phase7`, `phase8`, `phase9-schema`, `phase9-seed`) were pushed via `supabase link` + `supabase db push`.
3. **This surfaced a real, pre-existing bug in the phase 7 migration** (not introduced this session): `public.is_admin()` is a `language sql` function, which Postgres validates against the catalog at *creation* time (unlike `plpgsql`, which defers checks to first execution) — and it referenced `public.profiles` before that table existed in the file's original ordering. This had apparently never been tested against a real clean Postgres database before. Fixed by moving the function definition to after the `profiles` table. **The exact same latent bug existed in this session's own `is_enrolled_in_masterclass_cohort`/`is_enrolled_in_masterclass_program` functions** (phase 9) and was fixed the same way — moved to after `masterclass_cohorts` is created. Both fixes are in the committed migration files now; re-running `supabase db push` against a fresh project applies cleanly.
4. Verified against the live database directly (not just "it didn't error"): the `courses`/`masterclass_programs`/`masterclass_weeks` rows read back correctly, and — importantly — an anonymous request against `masterclass_terminology` (gated content) correctly returned an empty array rather than an error, confirming the RLS design works against a real database, not just in review.
5. `VITE_ENABLE_SUPABASE_AUTH=true` is now set, switching the whole app from local mock auth to real Supabase auth. **The old local bootstrap test accounts no longer work.** A real admin account was created via the Supabase Admin API (email/password supplied directly by the user, `user_metadata.role: "admin"` set at creation so the `handle_new_user_profile()` trigger assigned `profiles.role = 'admin'` automatically) — verified by both a direct database read and a real browser login reaching `/admin` successfully.
6. `VITE_ADMIN_EMAILS` is set to the real admin's email. No other new environment variables were needed — this feature reused the existing var names throughout.
7. No new payment configuration was needed — the masterclass cohort reuses the existing KCB Paybill config in `src/data/lmsConfig.ts` verbatim (the real Paybill number/account should still be confirmed as non-placeholder before advertising paid enrollment — see README "What else is actually needed").
8. The service_role/secret API key used for the one-time admin-account creation was used transiently in a single command and is **not** stored in `.env`, the repo, or any committed file — see `credentials.md` (git-ignored) for where to find it again if needed.
...
- The real payment → admin-approval → unlock flow was verified by construction (it reuses untouched, previously-shipped code) and the underlying tables/RLS are now confirmed live and correct, but a full submit-a-payment-and-approve-it click-through was not performed in this pass — worth a manual pass before the first real student pays.
